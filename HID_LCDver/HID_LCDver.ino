...
int lastState_FSW1 = LOW;
int currentState_FSW1;
unsigned long pressedTime_FSW1  = 0;
unsigned long releasedTime_FSW1 = 0;

//Foot switch 2 debounce________________________________________________(3.1)
int lastState_FSW2 = LOW;                             
int currentState_FSW2;                               
unsigned long pressedTime_FSW2  = 0;                  
unsigned long releasedTime_FSW2 = 0;                  
...
void setup() {
  ...
  pinMode(footSwitch1, INPUT_PULLUP);
...
void loop() {
  ...
  if (lastState_FSW1 == HIGH && currentState_FSW1 == LOW) {      // button is pressed
    pressedTime_FSW1 = millis();
  }
  else if (lastState_FSW1 == LOW && currentState_FSW1 == HIGH) { // button is released
    releasedTime_FSW1 = millis();

    long pressDuration_FSW1 = releasedTime_FSW1 - pressedTime_FSW1;

    if ( pressDuration_FSW1 < SHORT_PRESS_TIME ) {
      Serial.println("A SHORT press is detected");
      foot_switch1();
    }
    if ( pressDuration_FSW1 > LONG_PRESS_TIME ) {
      Serial.println("A long press is detected");
    }
  }
  ...
  //Foot switch 2 debounce________________________________________________________(3.1)
//  currentState_FSW2 = digitalRead(footSwitch2);
//  if (lastState_FSW2 == HIGH && currentState_FSW2 == LOW) {      // button is pressed
//    pressedTime_FSW2 = millis();
//  }
//  else if (lastState_FSW2 == LOW && currentState_FSW2 == HIGH) { // button is released
//    releasedTime_FSW2 = millis();
//
//    long pressDuration_FSW2 = releasedTime_FSW2 - pressedTime_FSW2;
//
//    if ( pressDuration_FSW2 < SHORT_PRESS_TIME ) {
//      Serial.println("A SHORT press is detected");
//      foot_switch2();
//    }
//    if ( pressDuration_FSW2 > LONG_PRESS_TIME ) {
//      Serial.println("A long press is detected");
//    }
//  }
...
//foor switch 2 debounce v2________________________________________________________________
 currentState_FSW2 = digitalRead(footSwitch2);
  if (lastState_FSW2 == LOW && currentState_FSW2 == HIGH) {      // button is pressed
    pressedTime_FSW2 = millis();
  }
  else if (lastState_FSW2 == HIGH && currentState_FSW2 == LOW) { // button is released
    releasedTime_FSW2 = millis();
...
void profile_data() {
  switch (profileGroup) {
    ...
    case 2:                           //account manager
      switch (profileSelect) {
        case 0:                         //GMAIL
          lcd.setCursor(0, 1); lcd.print("                "); lcd.setCursor(0, 1); lcd.print("Gmail      ");

          break;
        case 1:                         //gelbooru
          lcd.setCursor(0, 1); lcd.print("                "); lcd.setCursor(0, 1); lcd.print("Gelbooru");

          break;
        case 2:                         //facebook
          lcd.setCursor(0, 1); lcd.print("                "); lcd.setCursor(0, 1); lcd.print("Facebook");
